...
-- How productive is each team based on completed work?

-- 2. What is the exact definition?
-- Completed tasks divided by the number of members in each team.

-- 3. What are the edge cases?
-- Teams with zero users could cause division by zero, so NULLIF is used.

-- 4. What is the unit?
-- Completed tasks per person.

-- 5. What would make this metric misleading?
-- A team with many small tasks could look better than a team with fewer but harder tasks.
...
-- 5. What would make this metric misleading?
-- Deadlines could be changed to make the results look better.
...
-- Exercise 3
-- 1. What is the business question?
-- Which teams are overloaded, healthy, or underutilized?

-- 2. What is the exact definition?
-- Total tasks, active tasks, and completion rate per team.
...
-- 5. What would make this metric misleading?
-- The number of tasks does not show task complexity.
...
-- Exercise 4
-- 1. What is the business question?
-- How fast are tasks being resolved?

-- 2. What is the exact definition?
-- Average, median, fastest, and slowest resolution time by priority.
...
-- 5. What would make this metric misleading?
-- Mixing all priorities together could hide critical delays.
...
SELECT
    ...
    ROUND(
        PERCENTILE_CONT(0.5)
        WITHIN GROUP (ORDER BY (completed_at - created_at) * 24),
    2) AS median_hours,
...
-- Exercise 5
-- 1. What is the business question?
-- Which tasks are overdue and how serious are they?

-- 2. What is the exact definition?
-- Non-completed and non-cancelled tasks where due_date has already passed.
...
-- 5. What would make this metric misleading?
-- It shows the delay but not the reason behind it.
...
-- Exercise 6
-- 1. What is the business question?
-- Which users are delivering the most valuable completed work?

-- 2. What is the exact definition?
-- Completed tasks weighted by priority, divided by active working days.

-- 3. What are the edge cases?
-- Users without completed tasks may generate null values, handled with NULLIF.

-- 4. What is the unit?
-- Weighted completed tasks per day.

-- 5. What would make this metric misleading?
-- It measures amount and priority, but not the quality of the work.
...
-- Exercise 7
-- 1. What is the business question?
-- What percentage of assigned work does each team complete?

-- 2. What is the exact definition?
-- Completed tasks divided by total assigned tasks per team.

-- 3. What are the edge cases?
-- Teams with zero tasks could cause division by zero.

-- 4. What is the unit?
-- Percentage.

-- 5. What would make this metric misleading?
-- A high completion rate does not always mean high business value.
...
-- Exercise 8
-- 1. What is the business question?
-- Which open tasks should be prioritized first?

-- 2. What is the exact definition?
-- Numeric urgency score based on priority and due date.

-- 3. What are the edge cases?
-- Tasks without due date cannot be scored correctly.

-- 4. What is the unit?
-- Numeric score.

-- 5. What would make this metric misleading?
-- The formula may not fully represent real business impact.
...
-- Part D
-- 1. What is the business question?
-- What is the overall health of the task system?

-- 2. What is the exact definition?
-- One summary row with total, completed, active, overdue, completion rate, resolution time,
-- overdue average, most common priority, and busiest team.

-- 3. What are the edge cases?
-- Cancelled tasks can distort completion rate, and ties may exist in priority or busiest team.

-- 4. What is the unit?
-- Counts, percentages, hours, days, and labels.

-- 5. What would make this metric misleading?
-- A good general completion rate could hide problems with critical tasks.
...
SELECT
    ...
    ROUND(
        AVG(CASE WHEN status = 'completed'
            THEN (completed_at - created_at) * 24 END),
    2) AS avg_resolution_hours,
    ROUND(
        AVG(CASE WHEN is_overdue = 1
            THEN TRUNC(SYSDATE - due_date) END),
    2) AS avg_days_overdue,
    (
        SELECT priority
        FROM priority_rank
        WHERE cnt = (SELECT MAX(cnt) FROM priority_rank)
        FETCH FIRST 1 ROW ONLY
    ) AS most_common_priority,
    (
        SELECT team_name
        FROM team_rank
        WHERE cnt = (SELECT MAX(cnt) FROM team_rank)
        FETCH FIRST 1 ROW ONLY
    ) AS busiest_team
FROM base;